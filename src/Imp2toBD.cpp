...
CWriteBD12::CWriteBD12(const std::string &_filename, int _numSub, int _numSNPs) : CWriteBD11(_filename, _numSub, _numSNPs) {}
...
int CWriteBD12::WriteSNP(const double *_d, const double *_p0, const double *_p1, const double *_p2) {
  ...
  for (i = 0; i < m_numSubjects; ++i, ++p1, ++p2, ++u1, ++u2) {
    up1 = (short)(*p1 * 0x7ffe);
    d1 = ((double)up1) / 0x7ffe;
    if (d1 > *p1)
      up2 = up1 - 1;
    else
      up2 = up1 + 1;
    d2 = ((double)up2) / 0x7ffe;
