...
int Imp2toBDC(const Rcpp::List &imp2Info, const std::string &filename, int format, int subformat) {
  ...
  } else if (format == 4 && subformat == 2) {
    bdFile = new CWriteBD42(filename, (int)imp2File->NumSubjects(), (int)imp2File->NumSNPs());
  } else {
...
CWriteBD42::CWriteBD42(const std::string &_filename, int _numSub, int _numSNPs) : CWriteBD(_filename, _numSub, _numSNPs) {}

int CWriteBD42::WriteHeader() {
...
int CWriteBD42::WriteSubjectAndSNPInfo(const Rcpp::List &imp2Info) {
  const int numGroups = 1;
  bool usesFID;
  int subjectOptions;
  int snpOptions;
...
int CWriteBD42::WriteSNP(const double *_d) {
  if (CWriteBD::WriteSNP(_d))
    return 1;
...
int CWriteBD42::WriteSNP(const double *_d, const double *_p0, const double *_p1, const double *_p2) {
  unsigned short *u1, *u2;
  const double *d, *p0, *p1, *p2;
  int snpSize;
  int i;
